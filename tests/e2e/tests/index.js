...
test("successful submission", async t => {
  await t
    .click(homePage.navbar.newArticleLink)
    .typeText(newArticlePage.title, "title")
    .typeText(newArticlePage.editor, "content")
    .click(newArticlePage.createArticleBtn)
    .expect(newArticlePage.articlePublishedModal.message.textContent)
    .eql("Yayyyy!")
    .expect(newArticlePage.articlePublishedModal.details.textContent)
    .eql("Your article has been published");
});
...
test("denied on validation failure TITLE AND BODY", async t => {
  await t
    .click(homePage.navbar.newArticleLink)
    .click(newArticlePage.createArticleBtn)
    .expect(newArticlePage.alert.textContent)
    .contains("Article Body, Title and Topic Information is required.");
});
...
test("present on show page", async t => {
  await t
    ...
    .expect(articlePage.articleTitle.textContent)
    .contains("title");
});
...
test("successful edit", async t => {
  await t
    ...
    .expect(editArticlePage.alert.textContent)
    .contains("Article has been successfully saved");
});
...
test("unsuccessful edit on validation failure WHAT CHANGED", async t => {
  await t
    ...
    .click(editArticlePage.updateButton)
    .expect(editArticlePage.alert.textContent)
    .contains("Article Body, Title, Topic and Change Info is required");
});
...
test("successful delete", async t => {
  await t
    ...
    .expect(editArticlePage.alert.textContent)
    .contains("Article has been deleted");
});
